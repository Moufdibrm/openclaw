...
Accept `mode` or provider intent values: `higgsfield`, `banana`, or `openai`.
Default to `higgsfield` when the user does not choose.
Use `banana` for fast exploratory/edit image work and `openai` for explicit OpenAI or premium image asks.
Keep all publication, external send, paid delivery mutation, and voice clone paths approval-gated.
