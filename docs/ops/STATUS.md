...
- `Mission Manager agent control contract`: `governance-side contract now defines MM-readable agent cards, control levels, action request schema, approval gates, profile materialization manifest, and no-double-truth source matrices without touching MM implementation`
...
- memory lifecycle is not production-closed: local KG JSONL is repaired, real processed DB replay is valid in no-apply mode, and copied-KG lifecycle E2E is green, but LLM Wiki `kg_operations` are not yet bridged into Jack-X review, Jack-X live commit is not review-gated, relation rendering is incomplete, and Jack retrieval proof after live memory update is missing
...
- `Jack X` durable memory production remains blocked until:
  ...
  - Jack-X has a feedback-loop route for later evidence, contradictions, staleness, and history-preserving corrections
...
- `Gateway control layer`: Jack runtime now has a narrow control plane for `status`, `health_check`, `restart_gateway`, `reload_config`, `release_status`, and `rollback_release`. VPS releases now switch through `current/previous` symlinks with health-gated rollback, while forward deploy remains operator-driven from the authoring machine. `/update` is disabled on BRM live deployments.
